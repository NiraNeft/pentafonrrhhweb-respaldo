...
// action
import { deleteRequisicion, resetAddRequisicionFlag } from "../../../../../slices/thunks";
import {
    TextInput,
...
const DeleteModal = ({ show, requisicion, onDeleteClick, onCloseClick }) => {
  const dispatch = useDispatch();
  
  // Creacion de requisicion
  if(!requisicion)
    requisicion = [];
  const validation = useFormik({
    ...
    initialValues: {
      encryptedId: requisicion.Id || "",
      userID: requisicion.userID || "1",
      detalle: "",
    },
    ...
    onSubmit: (setData) => {
      ...
      dispatch(deleteRequisicion(dataIn));
    }
  });
  ...
  const registerdatatype = createSelector(
      ...
      (Requisiciones) => ({
        requisicionDel: Requisiciones.requisicionDel,
  ...
  const {
      error, success, requisicionDel
  } = useSelector(registerdatatype);
  
  useEffect(() => {
      if (success && requisicionDel) {
        toast.success("Requisicion eliminada correctamente.", { autoClose: 3000 });
        dispatch(resetAddRequisicionFlag());
        onDeleteClick();
      }
  }, [dispatch, success, error, requisicionDel]);
